...
- ignore-requirements = will start a unit but only ignore the requirement dependencies\n\
  It is not recommended to make use of the latter two options."
    ...
    try {
        std::string return_value = this->getSystemdManager()->StartUnit(arguments.at(0), arguments.size() > 2 ? arguments.at(1) : "fail");
            ...
            if (registeredCommands.count(cmd) == 1) {
                ...
                try {
                    ...
                    case OK:
                        break;
                    case UNKNOWN_ERROR:
                        this->sendMessage(message.channelID, ":exclamation: unknown error!");
                        break;
